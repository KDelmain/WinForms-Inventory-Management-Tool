...
namespace KaitlynDelmain_C968
{
    ...
    {
        private BindingList<Part> tempPartList = new BindingList<Part>();
        ...
        {
            Product prod = new Product(Convert.ToInt32(idTB.Text), nameTB.Text, Convert.ToDecimal(priceTB.Text), Convert.ToInt32(inventoryTB.Text), Convert.ToInt32(minTB.Text), Convert.ToInt32(maxTB.Text));
            prod.assocPartList = tempPartList;
            Inventory.swapProd(prod);
        ...
        private void dgvAssocParts_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Inventory.currentPart = Inventory.ProdList[Inventory.prodIndex].assocPartList[Product.assocPartIndex];
        }
        ...
        {
            tempPartList = Inventory.currentProduct.assocPartList;
            ...
            dgvAssocParts.DataSource = Inventory.currentProduct.assocPartList;

            saveButton.Enabled = false;

            idTB.Text = Inventory.currentProduct.ProductID.ToString(); //fix
            idTB.Enabled = false;

            Inventory.partIndex = -1;
            Product.assocPartIndex = -1;

            nameTB.Text = Inventory.currentProduct.Name;
            inventoryTB.Text = Inventory.currentProduct.InStock.ToString();
            priceTB.Text = Inventory.currentProduct.Price.ToString();
            maxTB.Text = Inventory.currentProduct.Max.ToString();
            minTB.Text = Inventory.currentProduct.Min.ToString();
        }
    }
}
