...
namespace KaitlynDelmain_C968
{
    public partial class ModPart : Form
    {
        ...
        {
            idTB.Text = Inventory.currentPart.PartID.ToString();
            nameTB.Text = Inventory.currentPart.Name;
            inventoryTB.Text = Inventory.currentPart.InStock.ToString();
            priceTB.Text = Inventory.currentPart.Price.ToString();
            maxTB.Text = Inventory.currentPart.Max.ToString();
            minTB.Text = Inventory.currentPart.Min.ToString();

            if(Inventory.currentPart is Inhouse)
            {
                Inhouse ih = (Inhouse)Inventory.currentPart;
                inhouseOutsourcedTB.Text = ih.MachineID.ToString();
                isInhouse = true;
                inhouseRB.Checked = true;
            }
            else
            {
                Outsourced os = (Outsourced)Inventory.currentPart;
                inhouseOutsourcedTB.Text = os.CompanyName;
                isInhouse = false;
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            if (isInhouse)
            {
                Part part = new Inhouse(Convert.ToInt32(idTB.Text), nameTB.Text, Convert.ToDecimal(priceTB.Text), Convert.ToInt32(inventoryTB.Text), Convert.ToInt32(minTB.Text), Convert.ToInt32(maxTB.Text), Convert.ToInt32(inhouseOutsourcedTB.Text));
                Inventory.swapPart(part);
            }
            else
            {
                Part part = new Outsourced(Convert.ToInt32(idTB.Text), nameTB.Text, Convert.ToDecimal(priceTB.Text), Convert.ToInt32(inventoryTB.Text), Convert.ToInt32(minTB.Text), Convert.ToInt32(maxTB.Text), inhouseOutsourcedTB.Text);
                Inventory.swapPart(part);
            }
